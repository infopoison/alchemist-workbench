...
VALENCE_PROMPT_ANGLE_IN_SIGN = """
            You are an expert archetypal astrologer. Your task is to identify the core archetypal expressions, or "valences," for a zodiac sign on a chart angle by applying the specific astrological rule known as **The Adverbial Signature (Extended for Angles)**.

            **Interpretive Rule to Apply: The Adverbial Signature (Extended for Angles)**

            * **Core Principle:** The sign on a chart angle acts as an "adjective" or adverbial signature, describing the **tone, style, and conditions** one experiences in that foundational area of life. The Angle represents a crucial pillar of the self or life structure (Identity, Home/Roots, Partnership, or Public Life/Career).

            * **Governing System:** The interpretation is generated by modifying the Angle's core domain with the archetypal qualities of the sign on its cusp. For example, Ascendant in Aries suggests an assertive and pioneering approach to personal identity and self-presentation.

            **Your Task:**
            1.  Analyze the provided astrological components and the core principle of the interpretive rule. Apply **The Adverbial Signature (Extended for Angles)** rule to understand how the Sign's qualities modify the domain of the Angle. Identify the Angle's core domain (e.g., "Identity and Persona" for the Ascendant) as the **primary context**.
            2.  Generate a list of 3 distinct "expression archetypes" (valences) that represent the different potential manifestations of this energy. The valences should be ordered along a spectrum from most harnessed (positive valence) to most unharnessed (negative) manifestations of the energy.
            3.  Format the final output as a JSON object with a single key "valences". This key should contain a list of objects, where each object has two keys: "archetype" (the name of the valence, e.g., "The Spiritual Warrior") and "description" (a brief, one-sentence explanation of what this expression represents).
...
class PromptAssembler:
    ...
    def _determine_synthesis_type_and_rule(self, components: List[Dict[str, str]]) -> (str, str):
        ...
        if num_components == 2:
            ...
            if types == ['angle', 'zodiac_sign']: return 'angle_in_sign', "The Adverbial Signature (Extended for Angles)"
